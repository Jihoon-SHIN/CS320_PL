...
; bin-op : (symbol number number-> number) (symbol) (listof number) (listof number) -> (listof number)
...
(define-type PWAE
  ...
  [sub (left PWAE?) (rignt PWAE?)]
  [with (name symbol?) (init PWAE?) (body PWAE?)]
  [id (name symbol?)]
  [pooh (arr list?) (ar symbol?)])
...
(define (parse-sexpr sexp)
  (match sexp
    ...
    [(list 'pooh x ...) (pooh (map (lambda (y) (parse-sexpr y)) (drop-right x 1))
                              (car (drop-right (foldl cons '() x) (- (length (foldl cons '() x)) 1))))]
...
(define (subst expr from to)
  (type-case PWAE expr
    ...
    [pooh (arr ar) (pooh (map (lambda (x) (subst x from to)) arr) ar)]
...
(define (eval expr)
  (type-case PWAE expr
    ...
    [pooh (arr ar) (accumulate (map (lambda (x) (eval x)) arr) ar)]
    [with (bound-id named-expr bound-body)
...
; accumulate : listof number symbol(+ of -) -> number of listof number
; pooh를 계산하기 위해서는 재귀를 이용하여 덧뺄셈을 해야하므로 새로운 accumulate라는
; 함수를 만들어서 계산하기 용이하게 만들었다.
(define (accumulate arr ar)
  (define (calplus arr init)
    (if (null? arr)
        init
        (calplus (cdr arr) (bin-op + init (car arr)))))
  (define (calminus arr init)
    (if (null? arr)
        init
        (calminus (cdr arr) (bin-op - init (car arr)))))
  (if (eq? ar '+) (calplus (cdr arr) (car arr)) (calminus (cdr arr) (car arr))))
...
;; making own tests
(test (run "{pooh 1 2 3 4 5 -}") '(-13))
(test (run "{with {y {with {y 20} {pooh 1 y +}}} {with {x 10} {pooh x y -}}}") '(-11))
